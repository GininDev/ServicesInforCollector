...
namespace ServicesInforCollector.Core.Helpers
{
    public class ConfigHelper
    {
            ...
            string cv)
        {
            ...
            if (bIsXml)
            {
                try
                {
                    ...
                    string dstPath = string.Format("{0}{1}Signal{1}{2}", dBase, Path.DirectorySeparatorChar, s2);
                    ...
                    if (arDlls != null && arDlls.Any())
                    {
                        ...
                        var arRec = new Dictionary<string, string> {{"ServerName", sIp}, {"Port", sPort}};
                        if (arConfigs == null)
                            arConfigs = new Dictionary<string, object>();
            ...
            else
            {
                ...
                var arKeys = SmartSection(objId);
                string sVirt;
                var sVals = ReadProperties(objId, arKeys, out sVirt);
        ...
        /// <returns></returns>
        private static Dictionary<int, Dictionary<string, string>> ReadProperties(IniDocumentHelper objId, string[] arKeys, out string sVirt)
        {
            ...
            for (int i = 0; i < arKeys.Length; i++)
            {
                ...
                {
                    var aVirt = string.Join(":", arVirt.ToArray());
                    if (arOut == null)
